...
const ChildId = (props) => {
  ...
  // 부모로 부터 전달 받은 idHandler 함수 -> 부모의 상태 변수 값을 자식 input에 입력된 값으로 변경하는 함수 실행
...
const ParentComponent = () => {
  ...
  const idHandler = e => {
    // e : 이벤트 객체
    setId(e.target.value); // input에 작성된 값이 변하면 상태 변수 id 값을 바꿔서 리 랜더링 수행
  }
  const pwHandler = e => {
    setPW(e.target.value);
  }
  // 상태 끌어올리기로 자식한테서 얻어온 값이 user01., pass01이 맞는지 확인
  const loginCheck = () => {
