...
const StateReview1 = () => {
  ...
  const [strList, setStrList] = useState([]); // 상태변수로 배열 선언
  ...
  const enterHandler = (e) => {
    ...
    if(e.key === 'Enter') { // 엔터 키가 입력된 경우
      // 상태변수 setList의 값을 변경
      // setStrList([e.target.value]); // 아래처럼 작성 하거나 현재 작성된 구문처럼 작성해도 됨
      setStrList([...strList, str]);
      e.target.value = ''; // input에 작성된 값 삭제
      setStr(''); // 상태변수에 '' 대입 -> 상태변수 값 변경 시 리랜더링
    }
  }
  ...
  return(
    <div>
      ...
      <div>
        ...
        <input type="text" id="inputString" onChange={inputStringHandler} onKeyUp={enterHandler} />
      </div>
      {/* 위 input에 입력된 값이 출력됨 */}
      <h3>입력된 값 : {str} </h3>
...
/* 배열.map((item, index) => {}) : 매개변수에 작성된 함수에서 반환된 값을 이용해 새로운 배열을 만드는 배열 매서드
